...
@invocation(
    "tiled_multi_diffusion_ip_denoise_latents",
    title="Tiled IP-Friendly Multi-Diffusion Denoise - SD1.5, SDXL",
    tags=["upscale", "denoise"],
    category="latents",
    classification=Classification.Beta,
    version="1.0.1",
)
class TiledMultiDiffusionIPDenoiseLatents(BaseInvocation):
    """Tiled Multi-Diffusion denoising with IP-Adapter.
    ...
    This node has a similar interface to the `DenoiseLatents` node, but it has a reduced feature set (T2I-Adapter,
    masking, etc.).
    """
    ...
    def invoke(self, context: InvocationContext) -> LatentsOutput:
        ...
        with (
            ...
            ip_adapter_data = DenoiseLatentsInvocation.prep_ip_adapter_data(
                context=context,
